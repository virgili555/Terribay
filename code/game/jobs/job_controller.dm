...
/datum/controller/occupations
	...
	proc/SetupOccupations(var/faction = "Station", var/setup_titles = 0)
		...
		for(var/J in all_jobs)
			...
			if(job.department_flag & HIGHNOBLE)
				highnoble_positions |= job.title
			if(job.department_flag & LOWNOBLE)
				lownoble_positions |= job.title
			if(job.department_flag & CITYWATCH)
				citywatch_positions |= job.title
			if(job.department_flag & CITIZENRY)
				citizenry_positions |= job.title
