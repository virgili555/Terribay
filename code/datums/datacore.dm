...
/datum/datacore/proc/get_manifest(monochrome, OOC)
	var/list/highnoble = new()
	var/list/lownoble = new()
	var/list/citywatch = new()
	var/list/citizenry = new()
	var/list/heads = new()
	var/list/spt = new()
	var/list/sec = new()
	var/list/eng = new()
	...
	for(var/datum/data/record/t in data_core.general)
		...
		var/department = 0
		if(real_rank in highnoble_positions)
			highnoble[name] = rank
			department = 1
		if(real_rank in lownoble_positions)
			lownoble[name] = rank
			department = 1
		if(real_rank in citywatch_positions)
			citywatch[name] = rank
			department = 1
		if(real_rank in citizenry_positions)
			citizenry[name] = rank
			department = 1
	...
	if(highnoble.len > 0)
		dat += "<tr><th colspan=3>High Nobles</th></tr>"
		for(name in highnoble)
			dat += "<tr[even ? " class='alt'" : ""]><td>[mil_ranks[name]][name]</td><td>[heads[name]]</td><td>[isactive[name]]</td></tr>"
			even = !even
	if(lownoble.len > 0)
		dat += "<tr><th colspan=3>Low Nobles</th></tr>"
		for(name in lownoble)
			dat += "<tr[even ? " class='alt'" : ""]><td>[mil_ranks[name]][name]</td><td>[heads[name]]</td><td>[isactive[name]]</td></tr>"
			even = !even
	if(citywatch.len > 0)
		dat += "<tr><th colspan=3>City Watch</th></tr>"
		for(name in citywatch)
			dat += "<tr[even ? " class='alt'" : ""]><td>[mil_ranks[name]][name]</td><td>[heads[name]]</td><td>[isactive[name]]</td></tr>"
			even = !even
	if(citizenry.len > 0)
		dat += "<tr><th colspan=3>Citizenry</th></tr>"
		for(name in citizenry)
			dat += "<tr[even ? " class='alt'" : ""]><td>[mil_ranks[name]][name]</td><td>[heads[name]]</td><td>[isactive[name]]</td></tr>"
			even = !even
	if(heads.len > 0)
